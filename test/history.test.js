...
describe("history", () => {
  ...
  it("should return failure if not good card", () => {
    return api.history(card_notgood).then(
  ...
  it("should return book history if good card", () => {
    return api.history(card_good).then(
      (data) =>
        assert.deepEqual(data.history[0], {
          record: "b1116804",
          author: "Peeters, Benoît, 1956-",
          checkedout: "2019-12-29",
          title:
            "Le théorème de Morcom / [illustrations de] Goffin sur un scénario de Peeters avec la collaboration graphique d'Etienne Schreder.",
        }),
